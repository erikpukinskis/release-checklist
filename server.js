...
library.using(
  ...
  function(releaseChecklist, element, WebSite, BrowserBridge, tellTheUniverse, basicStyles) {
    ...
    var newListBridge = new BrowserBridge()

    site.addRoute(
      "get",
      "/release-checklist",
      newListBridge.sendPage([basicStyles, storyForm])
    )
    ...
    function renderChecklist(list, response) {
      ...
      bridge.sendPage(form)(null, response)
    }
